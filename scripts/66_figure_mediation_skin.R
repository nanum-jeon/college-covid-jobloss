...
# 0) Data prep
# ----------------------------
skin_df <- covid_jobloss_results$main_results %>%
  filter(Group == "Light_Black" | Group == "Dark_Black") %>% 
  mutate(
    Group = dplyr::recode(Group,
                          "Light_Black" = "Light Black",
                          "Dark_Black"  = "Dark Black",
                          .default = Group),
    Group = factor(Group, levels = c("Dark Black", "Light Black")),
...
final_plot_by_skin <- cowplot::plot_grid(
  row_plots, 
  #shared_x_label,
  leg,
  ncol = 1, 
...
ggsave(
  filename = "figure_mediation_effect_by_skin.png",
  plot     = final_plot_by_skin,
...
cat(
  "Saved to:",
  normalizePath(file.path(DIR_RESULTS, "figure", "figure_mediation_effect_by_skin.png"), mustWork = FALSE),
  "\n"
)