...
covid_jobloss_effect_proportion <- calculate_proportion(covid_jobloss_results$main_results)


# Combine tables and generate LaTeX table 
covid_jobloss_regression_table <- create_regression_table(covid_jobloss_results$main_result)
 

covid_jobloss_latex_table <- covid_jobloss_regression_table %>%
  bind_rows(covid_jobloss_effect_proportion) %>%
  kbl(format = "latex", 
        booktabs = TRUE, 
        col.names = c("", 
        ...
        escape = FALSE, 
        linesep = "", 
        label = "covid-jobloss-effects",
        caption = "College Effects on COVID-19 Job Loss by Race and Skin Color: Total, Indirect Through High-Skilled Occupations, and Direct Effects") %>%
...
covid_jobloss_latex_table <- covid_jobloss_latex_table %>%
  footnote(
...
covid_jobloss_latex_table

# --- Write out ---
 
out_tex <- file.path(DIR_RESULTS, "table", "table_mediation_results.tex")
writeLines(covid_jobloss_latex_table, out_tex)
