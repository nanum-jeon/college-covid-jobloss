...
tab <- table_data %>%
  kbl(
    format   = "latex",
    caption  = "Group-specific Means in College Completion and Background Characteristics by Race and Skin Color",
    booktabs = TRUE,
    col.names = c("",
    ...
    align    = c("l", rep("r", 5)),
    label = "descriptives_all",
    escape   = FALSE,
    row.names = FALSE
  ) %>%
  # S columns, 1.8 cm each — brackets escaped
  column_spec(2:6, latex_column_spec = "S\\[table-format=1.3,table-column-width=1.5cm\\]") %>%
...
# now write out
out_tex <- file.path(DIR_RESULTS, "table", "table_descriptive_all.tex")
writeLines(tab_all, out_tex)
